...
describe("the plan", () => {
  it("is 10 keys at $25, 100 at $35, then $49 for good", () => {
    expect(TIERS.map((t) => [t.price, t.cap])).toEqual([
      [25, 10],
      [35, 100],
      [49, null],
    ]);
    expect(LIST_PRICE).toBe(49);
  });
...
describe("pricingSnapshot", () => {
  it("before launch, shows the plan with the first step current", () => {
    ...
    expect(s.tiers.map((t) => t.label)).toEqual(["keys 1-10", "keys 11-110", "from key 111"]);
  ...
  it("counts down the current step", () => {
    ...
    expect(s.current).toMatchObject({ key: "launch1", price: 25, sold: 7, left: 3 });
  });
  ...
  it("moves on when a step is sold out", () => {
    ...
    expect(s.current).toMatchObject({ key: "launch2", price: 35, left: 58 });
  });

  it("lands on the list price once every launch key is gone", () => {
    const s = pricingSnapshot([
      { key: "launch1", sold: 10 },
      { key: "launch2", sold: 100 },
      { key: "list", sold: 3 },
    ]);
    expect(s.current).toMatchObject({ key: "list", price: 49, left: null, status: "current" });
  });
  ...
  it("leaves out a launch step Polar has no discount for - the checkout could not sell it", () => {
    ...
    expect(s.current).toMatchObject({ key: "launch2", label: "keys 1-100", left: 95 });
  });
...
describe("tierLabel / usd", () => {
  it("labels ranges", () => {
    expect(tierLabel(11, 100)).toBe("keys 11-110");
    expect(tierLabel(111, null)).toBe("from key 111");
  });
